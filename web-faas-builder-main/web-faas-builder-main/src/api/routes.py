...
def _normalize_registry_credentials(registry_url: str, username: str | None, password: str | None) -> tuple[str | None, str | None]:
    """ECR + IRSA 기본 크리덴셜(dummy)을 비워 IRSA 경로를 타게 정규화."""
    if "ecr" in registry_url and "amazonaws.com" in registry_url:
        if password in (None, "", "dummy-password"):
            return None, None
...
def run_push_task(
    ...
    try:
        ...
        if real_client.is_configured() and s3_source_path:
            ...
            try:
                norm_user, norm_pass = _normalize_registry_credentials(registry_url, username, password)
